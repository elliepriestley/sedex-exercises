...
class GrowthOfAPopulation {
    fun predictYear(
        ...
        if (startingPopulation >= targetPopulation) {
            ...
            val nextYearPopulation = nextYearPopulation(startingPopulation, percentageIncrease, additionalPeople)
            return 1 + predictYear(nextYearPopulation, percentageIncrease, additionalPeople, targetPopulation )
        }


        // starting Population = nextYearPopulation


    }

    fun nextYearPopulation(
        startingPopulation: Int,
        percentageIncrease: Float,
        additionalPeople: Int
    ): Int {
        // i.e 100 + 100 + (100/100) + 0 = 200
        val result = startingPopulation + startingPopulation*(percentageIncrease/100)
        return result.toInt()
    }
